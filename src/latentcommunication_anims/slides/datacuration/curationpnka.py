...
class CurationPNKA(Slide):
    def construct(self):
        ...
        pnka = Group(
            pnka,
            Tex("PNKA is essentially comparing relative representations sample-wise")
            .scale(0.75)
            .to_edge(DOWN),
        )
        ...
        tldr = (
            Group(
                Tex("tldr."),
                Tex(
                    r"Models disagree on samples with\\different relative representations"
                ),
                ImageMobject(
                    PROJECT_ROOT / "data" / "datacuration" / "pnka_plot"
                ).scale(0.8),
            )
