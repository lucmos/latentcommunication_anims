    ...
    def construct(self):
        ...
        top_authors = VGroup(
            *[
                Tex(x, font_size=40)
                for x in (
                    r"\textit{Luca Moschella}",
                )
            ]
        ).to_corner(DL).set_opacity(0.5)
        ...
        sapienzalogo = (
            ImageMobject(PROJECT_ROOT / "data" / "logos" / "sapienza_logo.png")
            .scale(0.055)
        )
        gladialogo = (
            ImageMobject(PROJECT_ROOT / "data" / "logos" / "logo_gladia_white.png")
            .rescale_to_fit(length=sapienzalogo.height, dim=1)
        )
        ista_logo = (
            ImageMobject(PROJECT_ROOT / "data" / "logos" / "ista_logo.png")
            .rescale_to_fit(length=sapienzalogo.height, dim=1)
        )
        iclr_logo = (
            ImageMobject(PROJECT_ROOT / "data" / "logos" / "iclr_logo.png")
            .rescale_to_fit(length=sapienzalogo.height, dim=1)
        )
        neurips_logo: ImageMobject = (
            ImageMobject(PROJECT_ROOT / "data" / "logos" / "neurips_logo.png")
            .rescale_to_fit(length=sapienzalogo.height, dim=1)
        )
        logos = Group(
            gladialogo, sapienzalogo, ista_logo, iclr_logo, neurips_logo
        ...
        self.play(
            AnimationGroup(
                *(FadeOut(x, shift=DOWN, run_time=1.5) for x in (Group(top_authors, logos), paper_title_sub2, paper_title_sub, paper_title)),
                run_time=2,
                lag_ratio=0.2,
            ),
        )
