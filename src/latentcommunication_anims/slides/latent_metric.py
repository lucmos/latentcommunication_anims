...
def update_values(
    ...
    dot = Dot(point=axes.coords_to_point(sample_df.similarity, sample_df.score), radius=dot_radius)
...
class LatentMetric(Slide):
    def animate_a_dot(self, dot, reference_values, new_values, run_time=1.5):
        self.play(
            ...
            AnimationGroup(
                *(ShowPassingFlash(Underline(x, color=YELLOW)) for x in new_values.values()),
                *(Transform(reference_values[key], target_tex) for key, target_tex in new_values.items()),
            ),
            run_time=run_time,
        )
    ...
    def construct(self):
        ...
        for x, c in zip(g, COLORS):
            node_style_anim.append(x.animate.set_style(stroke_color=c, stroke_opacity=0.5))
        ...
        good_model = (
            VGroup(given_model, ref_model.next_to(given_model, DOWN, buff=MED_LARGE_BUFF))
            .move_to(ORIGIN)
            .to_edge(LEFT, buff=LARGE_BUFF)
        )
        self.play(Create(good_model), Uncreate(cora_dataset), g.animate.shift(RIGHT * 2))
        ...
        hyperparms = {key: Tex(name, font_size=PARAM_FONT_SIZE) for key, name in COLUMN2NAME}
        params = (
            VGroup(*hyperparms.values()).arrange(DOWN, aligned_edge=RIGHT, buff=0.3).move_to(ORIGIN).shift(RIGHT * 3)
        )
        ...
        x_label = Tex("Relative latent similarity to ", "$\mathcal{M}$", font_size=54).next_to(
            axes.x_axis, DOWN, buff=MED_LARGE_BUFF
        )
        y_label = Tex("Performance", font_size=54).rotate(PI / 2).next_to(axes.y_axis, LEFT, buff=MED_LARGE_BUFF)
        VGroup(axes, x_label, y_label).scale(0.9).move_to(ORIGIN).to_edge(LEFT, buff=MED_LARGE_BUFF)

        reference_dot, hypervalues, hypertexts = update_values(
            axes=axes, rows_refs=hyperparms, points_df=exp_points, reference=True, exp_id=0
        )
        ...
        self.play(
            ...
            *(ShowPassingFlash(Underline(x, color=YELLOW)) for x in hypervalues.values()),
        )
        ...
        self.animate_a_dot(dot=new_dot, reference_values=hypervalues, new_values=new_hypervalues, run_time=2)
        ...
        for i in chosen_idxs:
            ...
            self.animate_a_dot(dot=new_dot, reference_values=hypervalues, new_values=new_hypervalues, run_time=1.5)
        ...
        self.play(
            AnimationGroup(FadeOut(right_objects, shift=RIGHT), left_objects.animate.move_to(ORIGIN), lag_ratio=0.25),
            run_time=2,
        )
        ...
        for x in added_dots:
            dots_size_anim.append(
                Transform(x, Dot(x.get_center(), color=x.color, radius=0.05, fill_opacity=x.fill_opacity))
            )
        ...
        self.play(
            *(
                Uncreate(x)
                for x in [dashed_trendline, *added_dots, x_label, y_label, axes, left_objects, corr_trendline]
            )
        )
        ...
        self.wait()
        self.next_slide()
        self.play(implication.animate.shift(LEFT), GrowArrow(not_implication, shift=RIGHT))
        self.play(Create(cross))

        self.wait()
        self.next_slide()
        self.play(*(Uncreate(x) for x in [implication, not_implication, similar_model, performing, cross]))

        # correlation_over_time = Tex(r"...zooming into\\[1.5ex]a single model training")
        # self.play(Create(correlation_over_time))

        performance_label = VGroup((d := Dot()), Tex("Performance", font_size=38).next_to(d))
        performance_label.set_color(PERF_COLOR)
        similarity_label = VGroup((d := Dot()), Tex("Similarity", font_size=38).next_to(d))
        similarity_label.set_color(SIM_COLOR)
        VGroup(performance_label, similarity_label).arrange(RIGHT, aligned_edge=UP, buff=LARGE_BUFF).to_edge(UP)
        ...
        self.next_slide()
        self.play(Uncreate(epoch_num), Uncreate(x_label), *(FadeOut(x) for x in [axes, acc_graph, sim_graph, corr]))
        val_kwargs = {
            "line_color": PERF_COLOR,
            "add_vertex_dots": False,
            "stroke_width": 4,
        ...
        for idx_exp, i in enumerate(exps[: ROWS * COLS]):
            axes, acc_graph, sim_graph, objs, anims, exp_df, corr = parallel_plot(
                df,
                available_experiments[i],
            ...
            all_acc.append(acc_graph)
            all_sim.append(sim_graph)
            all_plots.append(objs)
            all_anims.extend(anims)
            all_corr.append(corr)

        grid_hack = (
            VGroup(*(Dot() for _ in range(ROWS * COLS)))
            .arrange_in_grid(rows=ROWS, cols=COLS, buff=2)
            .to_edge(DOWN, buff=1.5)
        )
        for p, d in zip(all_plots, grid_hack):
            p.scale(0.5).move_to(d.get_center())
        ...
        self.next_slide(auto_next=True)
        self.play(
            *(Uncreate(x) for x in [similarity_label, performance_label, *all_axes, *all_acc, *all_sim, *all_corr]),
            run_time=1,
        )
        self.wait(0.1)
